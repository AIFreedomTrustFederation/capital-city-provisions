...
export default function SteakDeliveryPage(){return <main className="site page-flow">
  <section className="page-hero poster-frame"><div><p className="eyebrow">Steak Delivery</p><h1>Steak-night energy, stocked at home.</h1><p className="lead">Ribeye, filet, New York strip, sirloin, and bundle options planned around your ZIP and monthly budget.</p><div className="actions"><a href="/#quick-route">Check Delivery</a><a href="/catalog">View Catalog</a></div></div><img src="/images/launch-freezer-box-hero.webp" alt="Steak delivery"/></section>
  <section className="section"><p className="eyebrow">Cuts</p><h2>The classics, ready when you are.</h2><div className="catalog-grid">{cuts.map(cut=><article key={cut.name}><img src="/images/launch-freezer-box-hero.webp" alt={`${cut.name} steak`}/><h3>{cut.name}</h3><p>{cut.text}</p><a href="/#quick-route">Check Availability</a></article>)}</div></section>
