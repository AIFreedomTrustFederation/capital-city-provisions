...
export async function sendWithOptionalNodemailer(record:EmailRecord):Promise<SmtpSendResult>{
  ...
  try{
    ...
    if(!mod?.default?.createTransport&&!mod?.createTransport)return {configured:true,ok:false,message:'Nodemailer is not installed yet. Run npm install nodemailer @types/nodemailer and redeploy.'};
