...
type SaleLead={id:string;name:string;email:string;phone:string;address:string;zip:string;area:string;need:string;offer:string;value:number;distance:string;temperature:'hot'|'warm'|'watch';status:QueueStatus;note:string;driverRoutePlan?:string;ownerOverride?:string;deliveryZoneStatus?:string;deliveryZoneCity?:string;deliveryZoneCounty?:string;deliveryZoneRing?:string;deliveryZoneMinutes?:number|null;deliveryZonePriority?:number;deliveryZoneMessage?:string;deliveryZoneNotes?:string};
...
function zoneFields(zip:string){const zone=zipZone(zip);return {deliveryZoneStatus:zone.status,deliveryZoneCity:zone.city,deliveryZoneCounty:zone.county,deliveryZoneRing:zone.ring,deliveryZoneMinutes:zone.minutes,deliveryZonePriority:zone.priority,deliveryZoneMessage:zone.message,deliveryZoneNotes:zone.notes}}
function pitchFor(lead?:SaleLead){
  ...
  if(lead.deliveryZoneStatus==='manual-review')return `Keep it honest: "Your ZIP needs route review before we promise delivery. I can queue your interest and have the team confirm grouped delivery timing before anything is locked in."`;
  if(lead.deliveryZoneStatus==='edge-route')return `Group it first: "You are inside our one-hour edge area, so we should group nearby demand and confirm timing before dispatch."`;
  ...
  return `Keep it simple: "Your route is active or groupable nearby. I can reserve the right box size, note your protein preferences, and have the team confirm the final plan before delivery."`;
}
...
export default function DriverSalesRouteMode({memory}:Props){
  ...
  function addLead(){
    ...
    const zone=zoneFields(draft.zip||draft.address);
    const areaName=draft.area||zone.deliveryZoneCity||draft.zip||'Live route';
    const record:SaleLead={id:`SALE-${Date.now()}`,name:draft.name,email:draft.email,phone:draft.phone,address:draft.address,zip:draft.zip,area:areaName,need:draft.need||'Freezer box follow-up',offer:draft.offer,value:Number(draft.value||0),distance:zone.deliveryZoneMinutes!==null?`${zone.deliveryZoneMinutes} min from Rancho Cordova`:'Manual route review',temperature:draft.temperature,status:'queued',note:draft.note,...zone};
    setLeads(current=>[record,...current]);
    setSelectedLead(record.id);
    setAi({answer:pitchFor(record),loading:false});
    setNotice(`${record.name} added with ZIP zone ${record.deliveryZoneStatus}. Save it when ready.`);
  ...
  async function updateLead(id:string,status:QueueStatus){
    ...
    try{
      const response=await fetch('/api/ops/driver-sales',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:target.id,driver:memory.driver||'Driver',sourceStopId:activeStop?.id,sourceCustomer:activeStop?.customer,routeId:activeStop?.routeId,leadName:target.name,email:target.email,phone:target.phone,address:target.address,zip:target.zip,area:target.area,need:target.need,offer:target.offer,estimatedValue:target.value,status,temperature:target.temperature,note:target.note,deliveryZoneStatus:target.deliveryZoneStatus,deliveryZoneCity:target.deliveryZoneCity,deliveryZoneCounty:target.deliveryZoneCounty,deliveryZoneRing:target.deliveryZoneRing,deliveryZoneMinutes:target.deliveryZoneMinutes,deliveryZonePriority:target.deliveryZonePriority,deliveryZoneMessage:target.deliveryZoneMessage,deliveryZoneNotes:target.deliveryZoneNotes,ownerOverride:target.ownerOverride,driverRoutePlan:target.driverRoutePlan||routePlan})});
  ...
  async function askSalesAi(intent:string){
    ...
    const local=intent==='queue'?`Queue ${lead.name} as ${lead.offer}. Next action: confirm ZIP ${lead.zip}, ${lead.deliveryZoneStatus}, preferred proteins, and best callback time before promising delivery.`:pitchFor(lead);
