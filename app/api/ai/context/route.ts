...
export async function GET(request:Request){
  try{
    ...
    const context=labelContext({mode:'live',setupProfile:profile,orders:labelRows((orders||[]).slice(0,50),source),salesQueue:labelRows((salesLeads||[]).slice(0,30),source),operatorBrain,report:report?withContextTrust(report,source):report,trainingSummary:withContextTrust({generatedAt:trainingDataset?.generatedAt||null,recordCount:trainingDataset?.records?.length||0,records:(trainingDataset?.records||[]).slice(0,20).map((record:any)=>withContextTrust(record,'ai-summary',{reason:'Training sample derived from operational data for model/context review.'}))},'ai-summary',{reason:'Training summary is derived context, not a direct operational record.'}),permissions:['orders','reports','operator brain','zip heat','sales priorities','profit/loss','restock planning','route learning','driver updates','sales queue','training review'],blocked:['customer-facing claims that purchase improves giveaway odds'],message:hasDb?'Owner context is generated from PostgreSQL source-of-truth records and operator brain signals.':'Development fallback context; production should use PostgreSQL.'},source);
