...
export default function SteakDeliveryPage(){return <main className="site page-flow">
  <section className="page-hero poster-frame"><div><p className="eyebrow">Steak Delivery</p><h1>Steak-night energy, stocked at home.</h1><p className="lead">Ribeye, filet, New York strip, sirloin, and bundle options planned around your ZIP and monthly budget.</p><div className="actions"><a href="/#delivery-zone-check">Check Delivery</a><a href="/catalog">View Catalog</a></div></div><img src="/images/category-beef.svg" alt="Steak delivery"/></section>
  <section className="section"><p className="eyebrow">Cuts</p><h2>The classics, ready when you are.</h2><div className="catalog-grid">{cuts.map(cut=><article key={cut.name}><img src="/images/category-beef.svg" alt={`${cut.name} steak`}/><h3>{cut.name}</h3><p>{cut.text}</p><a href="/#delivery-zone-check">Check Availability</a></article>)}</div></section>
  ...
  <section className="cta poster-frame final-cta"><p className="eyebrow">Reserve</p><h2>Check your ZIP and request the cuts you want.</h2><p>Share preferred steaks, budget, and timing so sales can follow up with a useful plan.</p><div className="actions"><a href="/#delivery-zone-check">Check ZIP</a><a href="/contact">Contact Sales</a></div></section>
</main>}