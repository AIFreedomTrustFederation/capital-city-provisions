...
export function smtpConfigured(){return false}
...
export async function queueEmail(record:EmailRecord){return await saveEmailRecord({...record,status:'queued',provider:'owner-compose'})}
export async function markEmailSent(record:EmailRecord,providerMessageId='owner-confirmed-sent'){return await saveEmailRecord({...record,status:'sent',providerMessageId,sentAt:now()})}
export async function markEmailFailed(record:EmailRecord,error='send not completed'){return await saveEmailRecord({...record,status:'failed',metadata:{...(record.metadata||{}),error}})}

export async function sendEmailOpenTransport(record:EmailRecord){
  const queued={...record,status:'queued' as EmailStatus,provider:'owner-compose'};
  await saveEmailRecord(queued);
  return {configured:true,ok:false,message:'No-secret mode: message is queued for owner-controlled sending.',record:queued};
}
