...
function localCopilot(action:CopilotAction,stops:any[],states:Record<string,StopState>,activeStop:any,turnIn:Record<string,string>){
  ...
  if(action==='next')return nextStop?`Next best move: work ${nextStop.id} for ${nextStop.customer}. Box: ${nextStop.box}. ${nextStop.notes||'Confirm delivery details before arrival.'}`:'All visible stops are marked delivered. Start the turn-in and flag any owner follow-up.';
  if(action==='route')return `Today has ${stops.length} stops with ${openStops.length} still open and ${issueStops.length} needing attention. Prioritize call-ahead notes, premium boxes, and any partial/restock blocker before closing the route.`;
  if(action==='customerText')return activeStop?`Text draft: Hi, this is Capital City Provisions. Your ${activeStop.box} is on today's ${activeStop.routeName}. We are heading your way in the ${activeStop.window} window. Reply here with any gate, parking, or drop-off note.`:'Pick a stop first and I will draft the customer text.';
  if(action==='issue')return issueStops.length?`Issue summary: ${issueStops.map((stop:any)=>`${stop.id} ${stop.customer}: ${states[stop.id]?.fulfillment}; ${states[stop.id]?.issue||'needs review'}`).join(' | ')}`:'No active issue stops yet. If a cut is short, use Voice Issue and mark partial or restock-blocked.';
  return `Turn-in prep: delivered ${Object.values(states).filter(state=>state.status==='delivered').length}/${stops.length}, missed ${turnIn.missed||'0'}, rescheduled ${turnIn.rescheduled||'0'}, miles ${turnIn.milesDriven||'not entered'}. Add payments and owner follow-up before submitting.`;
}
...
export default function DriverMobileWorkflow({memory}:Props){
  ...
  async function askCopilot(action:CopilotAction,question?:string){
    ...
    try{
      ...
      setCopilot({question:prompt,answer:localCopilot(action,stops,states,activeStop,turnIn),loading:false});
    }
  }
  ...
  return <section className="section driver-mobile-workflow" id="mobile-route">
    ...
    <div className="driver-ai-cockpit" aria-label="Driver AI cockpit">
      <div className="ai-cockpit-main">
        <p className="eyebrow">Driver AI Co-Pilot</p>
        <h3>{copilot.loading?'Thinking through the route...':'Best next move'}</h3>
        <p>{copilot.answer||nextSuggestion}</p>
        <form onSubmit={e=>{e.preventDefault();askCopilot('route',copilot.question)}} className="ai-ask-row">
          <input value={copilot.question} onChange={e=>setCopilot({...copilot,question:e.target.value})} placeholder="Ask about this route, a stop, a note, or turn-in..." />
          <button type="submit">Ask</button>
        </form>
      </div>
      <div className="ai-chip-grid">
        <button onClick={()=>askCopilot('next')}>Next Stop</button>
        <button onClick={()=>askCopilot('route')}>Route Summary</button>
        <button onClick={()=>askCopilot('customerText')}>Draft Text</button>
        <button onClick={()=>askCopilot('issue')}>Issue Review</button>
        <button onClick={()=>askCopilot('turnIn')}>Turn-In Prep</button>
      </div>
      <div className="ai-route-stats"><span><b>{openStops}</b> open</span><span><b>{money(routeValue)}</b> scheduled</span><span><b>{memory.driver||'Driver'}</b> assigned</span></div>
    </div>
