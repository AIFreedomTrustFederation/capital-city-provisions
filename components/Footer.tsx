...
const sections = [
  {
    ...
    links: [
      ...
      ['Monthly Restock Club', '/monthly-restock'],
      ...
      <div className="footer-inner">
        ...
        <nav className="footer-grid" aria-label="Footer navigation">
          {sections.map((section) => (
            <section className={open[section.title] ? 'footer-column open' : 'footer-column'} key={section.title} aria-label={section.title}>
              ...
              <ul>{section.links.map(([label, href]) => <li key={href}><a href={href}>{label}</a></li>)}</ul>
            </section>
          ))}
        </nav>
      </div>
