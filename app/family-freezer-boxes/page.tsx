...
export default function FamilyFreezerBoxesPage(){return <main className="site page-flow">
  <section className="page-hero poster-frame"><div><p className="eyebrow">Family Boxes</p><h1>Keep the week fed before it gets busy.</h1><p className="lead">A smarter stock-up for households that want dinner options ready, without building every meal from scratch at the last minute.</p><div className="actions"><a href="/#delivery-zone-check">Check ZIP</a><a href="/freezer-boxes">Compare Boxes</a></div></div><img src="/images/freezer-family.png" alt="Family box delivery"/></section>
  <section className="section"><p className="eyebrow">Lineup</p><h2>Choose the level that matches your home.</h2><div className="box-grid detail-box-grid">{boxes.map(box=><article key={box.name} className="marble box-card"><img src={box.image} alt={`${box.name} box`}/><div className="box-card-copy"><p className="eyebrow">{box.fit}</p><h3>{box.name}</h3><p className="box-size">{box.budget}</p><ul>{box.items.map(item=><li key={item}>{item}</li>)}</ul><a href="/#delivery-zone-check">Check Delivery First</a></div></article>)}</div></section>
  ...
  <section className="cta poster-frame final-cta"><p className="eyebrow">Build My Box</p><h2>Start with the delivery ZIP.</h2><p>Your saved ZIP helps the concierge keep every recommendation grounded in your area.</p><div className="actions"><a href="/#delivery-zone-check">Check ZIP</a><a href="/contact">Ask For Help</a></div></section>
</main>}