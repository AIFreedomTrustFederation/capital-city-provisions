...
const starterLeads:SaleLead[]=[
  {id:'SALE-2101',name:'R. Walker',email:'walker@example.com',phone:'916-555-2101',address:'Roseville CA',zip:'95661',area:'Roseville',need:'Family freezer restock',offer:'Family Box + cheesecake thank-you',value:525,distance:'0.8 mi from current stop',temperature:'hot',status:'queued',note:'Asked about weeknight meals and ground beef.'},
  {id:'SALE-2102',name:'Sierra Prep Kitchen',email:'buyer@example.com',phone:'916-555-2102',address:'Roseville CA',zip:'95678',area:'Roseville',need:'Recurring wholesale quote',offer:'Wholesale sample sheet',value:1180,distance:'1.4 mi from route',temperature:'hot',status:'queued',note:'Catering buyer wants simple protein pricing.'},
  {id:'SALE-2103',name:'D. Flores',email:'flores@example.com',phone:'916-555-2103',address:'Fair Oaks CA',zip:'95628',area:'Fair Oaks',need:'Starter freezer plan',offer:'Starter Box route hold',value:315,distance:'Queued for Tuesday route',temperature:'warm',status:'queued',note:'Price sensitive, likes chicken and sirloin.'},
  {id:'SALE-2104',name:'H. Bennett',email:'bennett@example.com',phone:'916-555-2104',address:'Folsom CA',zip:'95630',area:'Folsom',need:'Emergency meals',offer:'Food security plan consult',value:640,distance:'Waitlist route build',temperature:'watch',status:'queued',note:'Wants monthly provisioning after payday.'}
];
      ...
      <div className="sales-work-grid">
        ...
        <div className="sales-lead-stack">
          ...
          {leads.map(item=><article key={item.id} onClick={()=>setSelectedLead(item.id)} className={`${selectedLead===item.id?'active ':''}${item.temperature}`}>
            <div><span>{item.id}</span><h4>{item.name}</h4><p>{item.need}</p><small>{item.phone} - {item.email}</small></div>
        ...
        <aside className="sales-ai-panel">
          ...
          <div className="contact-grid">
            ...
            <a href={lead?.phone?`sms:${lead.phone}`:'#'}>Text</a>
            <a href={lead?.email?`mailto:${lead.email}`:'#'}>Email</a>
          </div>
          <textarea value={routePlan} onChange={event=>setRoutePlan(event.target.value)} placeholder="Plain-language route plan: go after this deal after the route, door knock nearby, or hook into tomorrow's route..." />
