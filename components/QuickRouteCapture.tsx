...
function nextStep(route:Route){if(route.status==='Confirmed route')return 'Expect route follow-up within 24 hours so CCP can match your freezer box.';if(route.status==='Almost full')return 'CCP will follow up quickly while your route is close to dispatch-ready.';if(route.status==='Join waitlist')return 'CCP will group your area with nearby customers and follow up when a route opens.';return 'CCP will use this route request to plan your freezer-box follow-up.'}
function pagePromptKey(){return `${PROMPT_PAGE_KEY}:${window.location.pathname}`;}
function saveZip(zip:string){const clean=(zip.match(/\d{5}/)?.[0]||'').trim();if(!clean)return '';localStorage.setItem(ZIP_STORAGE_KEY,clean);window.dispatchEvent(new CustomEvent('ccp:delivery-zip',{detail:{zip:clean}}));return clean;}
...
export default function QuickRouteCapture(){
  ...
  useEffect(()=>{const saved=localStorage.getItem(ZIP_STORAGE_KEY)||'';if(saved)setZip(saved);const currentPageKey=pagePromptKey();const maybePrompt=()=>{if(route||localStorage.getItem(ZIP_STORAGE_KEY)||sessionStorage.getItem(currentPageKey))return;sessionStorage.setItem(currentPageKey,'shown');setPrompt(true)};const timer=setTimeout(maybePrompt,10000);const onScroll=()=>{if(window.scrollY>520)maybePrompt()};window.addEventListener('scroll',onScroll,{passive:true});return()=>{clearTimeout(timer);window.removeEventListener('scroll',onScroll)}},[route]);

  function checkZip(e?:React.FormEvent){e?.preventDefault();const clean=saveZip(zip);if(!clean)return;setZip(clean);setRoute(routePlan(clean));setPrompt(false)}

  async function holdRoute(e:React.FormEvent){
    e.preventDefault();
    const clean=saveZip(zip);
    const plan=route||routePlan(clean);
    const lead={...details,address:clean,zip:clean,interest:'Quick route check',recommendation:'Freezer box route follow-up',route:plan.route,deliveryDay:plan.day,deliveryWindow:plan.window,routeStatus:plan.status,routeBadge:plan.badge,routeFill:plan.fill,routeCapacity:plan.capacity,routeReserved:plan.reserved,routeSlotsRemaining:plan.slotsRemaining,message:'Quick capture from landing page',source:'ccp-delivery-review',promoCode:'CHEESECAKE-48',couponOffer:'Free cheesecake with qualifying first freezer-box order within 48 hours of route check, while supplies last.',couponDeadlineHours:PROMO_HOURS,promoExpiresAt:promoExpiresAt(),giveawayAvailable:true,purchaseRequiredForGiveaway:false,purchaseImprovesGiveawayOdds:false};
    ...
    await Promise.allSettled([
      ...
      fetch('/api/delivery-review',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...lead,box:lead.recommendation,note:`${lead.name||'Customer'} requested CCP delivery review for ${plan.route}.`})})
    ]);
    setSent(true);
  }
