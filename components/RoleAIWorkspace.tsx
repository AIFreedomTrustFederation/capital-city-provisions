...
export default function RoleAIWorkspace({role,title,subtitle,memory}:WorkspaceProps){
  ...
  const routes=memory.routes||(memory.route?[memory.route]:[]);
  ...
  const aiContext=useMemo(()=>({role,memory:{...memory,routes,orders,turnIns},permissions:{customer:['own route','box guidance','promo clarity','giveaway rules'],driver:['assigned routes','stop notes','turn-ins'],owner:['all orders','all routes','reports','exports','route learning']}}),[role,memory,routes,orders,turnIns]);
  ...
  return <main className="site page-flow ops-shell">
    ...
    <section className="section" id="routes"><p className="eyebrow">Routes</p><h2>Route command board.</h2><div className="route-list ops-cards">{routes.map((route:any)=><article key={route.id||route.name}><h3>{route.name}</h3><p>{route.day} - {route.window}</p><strong>{route.status} - {route.reserved}/{route.capacity} grouped</strong><div className="ops-meter"><i style={{width:`${route.fill||Math.round((route.reserved/route.capacity)*100)}%`}}/></div><p>{route.priority||'Use ZIP and box details to confirm the next best step.'}</p>{(route.orders||[]).slice(0,3).map((order:any)=><small key={order.id}>{order.id}: {order.customer} - {order.box}</small>)}</article>)}</div></section>
