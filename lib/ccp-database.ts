...
export type OrderRecord={
  ...
  products:{sku:string;name:string;qty:number;unit:string;fulfilled:number;issue?:string}[];
  notes:string;
  promo?:string;
  createdAt:string;
  updatedAt:string;
...
export type LearningEvent={
  id:string;
  role:'customer'|'driver'|'owner'|'system';
  eventType:string;
  summary:string;
  signal:number;
  routeId?:string;
...
export type OwnerReport={
  ...
  restockIssues:number;
  routeEfficiency:{routeId:string;route:string;efficiency:RouteEfficiency;fuelUsed:number;milesDriven:number;profit:number}[];
  futureRestock:{product:string;needed:number;reason:string}[];
  ownerActions:string[];
...
export const dbCustomers:CustomerRecord[]=opsOrders.map((order,index)=>({
  id:`CUST-${1000+index}`,
  name:order.customer,
  email:`customer${index+1}@example.com`,
  phone:order.phone,
  zip:order.zip,
  source:order.promo?'promo-route-check':'route-check',
  preferences:order.proteins,
  createdAt:now
}));
...
export const dbRestockIssues:RestockIssue[]=[
  {id:'RI-1001',orderId:'CCP-1011',routeId:'rocklin-lincoln',sku:'FILET-2',product:'filet',needed:6,available:4,severity:'medium',action:'Offer filet substitution or hold 2 lbs for next restock.',createdAt:now}
];
...
declare global{var ccpDatabase:MutableDatabase|undefined}

export function getDatabase():MutableDatabase{
  if(!globalThis.ccpDatabase){
    globalThis.ccpDatabase={customers:[...dbCustomers],orders:[...dbOrders],driverUpdates:[...dbDriverUpdates],restockIssues:[...dbRestockIssues],learningEvents:[...dbLearningEvents]};
  }
  return globalThis.ccpDatabase;
}
...
export function getOrderLifecycle(orderId?:string){
  ...
  return orders.map(order=>({
    ...
    driverUpdates:db.driverUpdates.filter(update=>update.orderId===order.id),
    restockIssues:db.restockIssues.filter(issue=>issue.orderId===order.id),
    learningEvents:db.learningEvents.filter(event=>event.orderId===order.id||event.routeId===order.routeId)
  }));
}
...
export function createOrder(input:Partial<OrderRecord>&{customerName?:string;phone?:string;zip?:string}){
  ...
  if(!db.customers.find(customer=>customer.id===customerId)){
    db.customers.push({id:customerId,name:input.customerName||'New Customer',email:'',phone:input.phone||'',zip:input.zip||'',source:'order-intake',preferences:[],createdAt});
  }
...
export function applyDriverUpdate(input:Partial<DriverUpdate>&{orderId:string;routeId:string;driver:string}){
  ...
  if(update.restockIssue){
    db.restockIssues.unshift({id:`RI-${Date.now()}`,orderId:update.orderId,routeId:update.routeId,sku:'DRIVER-REPORTED',product:update.restockIssue,needed:1,available:0,severity:update.fulfillment==='restock-blocked'?'high':'medium',action:'Owner review required before next route promise.',createdAt});
  }
...
export function generateOwnerReport():OwnerReport{
  ...
  const routeEfficiency=opsRoutes.map(route=>{
    ...
    const efficiency=updates.find(update=>update.routeEfficiency==='poor')?'poor':updates.find(update=>update.routeEfficiency==='watch')?'watch':updates.find(update=>update.routeEfficiency==='excellent')?'excellent':'good';
    return {routeId:route.id,route:route.name,efficiency,fuelUsed,milesDriven,profit};
  });
  ...
  const ownerActions=[
    db.restockIssues.length?`Review ${db.restockIssues.length} restock issue(s) before promising premium boxes.`:'No active restock blockers in sample data.',
    partial.length?`Call ${partial.length} partial fulfillment customer(s).`:'No partial fulfillment calls needed.',
    'Use driver fuel/miles notes to adjust route grouping before dispatch.',
    'Convert learning events into route rules after owner review.'
  ];
...
export function aiTrainingDataset(){
  ...
  return {
    ...
    records:[
      ...db.orders.map(order=>({type:'order',input:`${order.zip} ${order.box} ${order.products.map(p=>p.name).join(' ')}`,output:`status=${order.status}; fulfillment=${order.fulfillment}; margin=${order.marginEstimate}; route=${order.routeId}`})),
      ...db.driverUpdates.map(update=>({type:'driver_update',input:`${update.routeId} ${update.status} ${update.fulfillment} ${update.restockIssue||''} miles=${update.milesDriven}`,output:`efficiency=${update.routeEfficiency}; learn=${update.customerNotes||update.partialReason||'no note'}`})),
      ...db.learningEvents.map(event=>({type:'learning',input:event.eventType,output:event.summary,signal:event.signal}))
    ]
  };
}
